...
import contracts_artifacts from '../../build/contracts/TokenCarfax.json'
		...
		c.get_client_list.call().then(function(clients){
			for(let i=0; i<clients.length; i++){
				c.get_client_tokens.call(clients[i]).then(function(num_tokens){
					str += '<tr bgcolor="white"><td>' + clients[i].toString() + '</td><td align=center>' + num_tokens.toString() + '</td></tr>';
					$("#clients").html(str);
				})
			}
		})
...
function disp_tokens(){
	...
	Contract.deployed().then(function(c){
		c.get_current_token_count.call().then(function(N){
			c.get_token_price.call().then(function(ppt){
				c.get_current_client_tokens.call().then(function(T){
					str += 'My tokens: ' + T.toString() + '<br/>';
					str += 'Total tokens sold: ' + N.toString() + " @ " + web3.fromWei(ppt).toString() + " (ether) per token<br/>";
					web3.eth.getBalance(c.address, function(error, result) {
						str += ("Contract account balance: " + web3.fromWei(result.toString()) + " (ether) <br/>");
...
function disp_tokens_purchase_form(){
	let str = `
        <form id="buy_token">
        	Tokens to buy: 
