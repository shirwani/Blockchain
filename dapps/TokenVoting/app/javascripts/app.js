...
var Voting = contract(contracts_artifacts);
	...
	Voting.deployed().then(function(contractInstance) {
		contractInstance.voterDetails.call(address).then(function(v) {
			...
			for(let i=0; i < allCandidates.length; i++) {
				...
				tokens_spent += parseInt(votesPerCandidate[i]);
				//console.log(votesPerCandidate[i])
			}
			tokens_balance = tokens_bought - tokens_spent;
			//console.log("Tokens balance: " + tokens_balance)
			$("#tokens-balance").html("Tokens balance: " + tokens_balance);
		});
	});
}
